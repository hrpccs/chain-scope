...
#instance="2025-08-08_114307"
instance="latest"
results_paths = f'benchmark/results/nginx.two_nodes/{instance}/exec_cpu.out'

class TestNum(IntEnum):
    SAMP_1_NO_PROXY_ES = 1
    SAMP_1_PROXY_ES = 2
    SAMP_100_NO_PROXY_ES = 3
    SAMP_100_PROXY_ES = 4
    NO_SAMP_NO_PROXY = 5
    NO_SAMP_PROXY = 6
    IDLE_NO_PROXY = 7
    ...
    PLAIN_NO_PROXY = 9
    PLAIN_PROXY = 10
    SAMP_1_NO_PROXY_SPAN = 11
    SAMP_1_PROXY_SPAN = 12
    SAMP_10_NO_PROXY_SPAN = 13
    SAMP_10_PROXY_SPAN = 14
    SAMP_100_NO_PROXY_SPAN = 15
    SAMP_100_PROXY_SPAN = 16
    SAMP_1_NO_PROXY_EVENT = 17
    SAMP_1_PROXY_EVENT = 18
    SAMP_10_NO_PROXY_EVENT = 19
    SAMP_10_PROXY_EVENT = 20
    SAMP_100_NO_PROXY_EVENT = 21
    SAMP_100_PROXY_EVENT = 22
    SAMP_1_PROXY_BEYLA = 23
    SAMP_10_PROXY_BEYLA = 24
    SAMP_100_PROXY_BEYLA = 25
    SAMP_1_PROXY_OTLP = 26
    SAMP_10_PROXY_OTLP = 27
    SAMP_100_PROXY_OTLP = 28
    NO_SAMP_PROXY_DF = 29
...
def parse_results(results_path, n_tests) -> dict[int, dict[str, float]]:
    ...
    with open(results_path, 'r') as file:
        for line in file:
            ...
            # User space
            elif '[ChainScope' in line and ']:' in line:
                print(line)
                component = line.split('-')[0].replace('[ChainScope', '').replace(']:', '').strip().lower()
                node = f" ({line.split('-')[1].replace(']:', '').strip().lower()})" if '-' in line else ''
                userspace = f'{component}{node}'
                results_[current_test][userspace] = 0
            elif '[Deepflow' in line and ']:' in line:
                print(line)
                component = line.split('-')[0].replace('[Deepflow', '').replace(']:', '').strip().lower()
                node = f" ({line.split('-')[1].replace(']:', '').strip().lower()})" if '-' in line else ''
                userspace = f'{component}{node}'
                results_[current_test][userspace] = 0
            elif '[Beyla' in line and ']:' in line:
                print(line)
                component = line.split('-')[0].replace('[Beyla', '').replace(']:', '').strip().lower()
                node = f" ({line.split('-')[1].replace(']:', '').strip().lower()})" if '-' in line else ''
                userspace = f'{component}{node}'
                results_[current_test][userspace] = 0
            elif userspace and 'task-clock' in line:
                cpu = float(line.split()[-3])*100
                results_[current_test][userspace] = cpu
                userspace = None
            ...
            # Add content to current section
                ...
                prog = l[2][1:-1]
                cpu = float(l[0][:-1])
                results_[current_test][f'{process}:{prog}{":sub" if sub_prog else ""}'] = cpu
            elif 'TOTAL' in line:
                results_[current_test][f'{process}:total'] = float(line.split()[0][:-1])
...
# cpu bpf
deepflow = [results[TestNum.NO_SAMP_PROXY_DF]['web server:total'], results[TestNum.NO_SAMP_PROXY_DF]['proxy:total']]

data = [
    # web server
    [
        [results[TestNum.SAMP_1_PROXY_BEYLA]['web server:total'], results[TestNum.SAMP_1_PROXY_EVENT]['web server:total']],  # sampling 1
        [results[TestNum.SAMP_10_PROXY_BEYLA]['web server:total'], results[TestNum.SAMP_10_PROXY_EVENT]['web server:total']],   # sampling .1
        [results[TestNum.SAMP_100_PROXY_BEYLA]['web server:total'], results[TestNum.SAMP_100_PROXY_EVENT]['web server:total']]   # sampling .01
    ],
    # proxy
    [
        [results[TestNum.SAMP_1_PROXY_BEYLA]['proxy:total'], results[TestNum.SAMP_1_PROXY_EVENT]['proxy:total']],
        [results[TestNum.SAMP_10_PROXY_BEYLA]['proxy:total'], results[TestNum.SAMP_10_PROXY_EVENT]['proxy:total']],
        [results[TestNum.SAMP_100_PROXY_BEYLA]['proxy:total'], results[TestNum.SAMP_100_PROXY_EVENT]['proxy:total']]
    ]
]
...
plt.savefig(f'{plots_dir}/nginx.cpu.bpf.pdf')

plt.show()

# cpu system
deepflow = [results[TestNum.NO_SAMP_PROXY_DF]['agent (web server)'], results[TestNum.NO_SAMP_PROXY_DF]['agent (proxy)'], results[TestNum.NO_SAMP_PROXY_DF]['server']]

data = [
    # web server
    [
        [results[TestNum.SAMP_1_PROXY_BEYLA]['agent (web server)'], results[TestNum.SAMP_1_PROXY_EVENT]['agent (web server)']],  # sampling 1
        [results[TestNum.SAMP_10_PROXY_BEYLA]['agent (web server)'], results[TestNum.SAMP_10_PROXY_EVENT]['agent (web server)']],   # sampling .1
        [results[TestNum.SAMP_100_PROXY_BEYLA]['agent (web server)'], results[TestNum.SAMP_100_PROXY_EVENT]['agent (web server)']]   # sampling .01
    ],
    # proxy
    [
        [results[TestNum.SAMP_1_PROXY_BEYLA]['agent (proxy)'], results[TestNum.SAMP_1_PROXY_EVENT]['agent (proxy)']],
        [results[TestNum.SAMP_10_PROXY_BEYLA]['agent (proxy)'], results[TestNum.SAMP_10_PROXY_EVENT]['agent (proxy)']],
        [results[TestNum.SAMP_100_PROXY_BEYLA]['agent (proxy)'], results[TestNum.SAMP_100_PROXY_EVENT]['agent (proxy)']]
    ],
    # collector
    [
        [results[TestNum.SAMP_1_PROXY_BEYLA].get('controller', 0), results[TestNum.SAMP_1_PROXY_EVENT]['controller']],
        [results[TestNum.SAMP_10_PROXY_BEYLA].get('controller', 0), results[TestNum.SAMP_10_PROXY_EVENT]['controller']],
        [results[TestNum.SAMP_100_PROXY_BEYLA].get('controller', 0), results[TestNum.SAMP_100_PROXY_EVENT]['controller']],
    ]
]
...
plt.savefig(f'{plots_dir}/nginx.cpu.sys.pdf')
...
plt.savefig(f'{plots_dir}/nginx.cpu.sys_agents.pdf')

plt.show()

# throughput
plain_app = results[TestNum.PLAIN_PROXY]['RPS']
deepflow = results[TestNum.NO_SAMP_PROXY_DF]['RPS']
#sampling_values = [0.001, 0.01, 0.1, 1]
sampling_values = [0.01, 0.1, 1]
sampling_beyla_data = [results[TestNum.SAMP_100_PROXY_BEYLA]['RPS'], results[TestNum.SAMP_10_PROXY_BEYLA]['RPS'], results[TestNum.SAMP_1_PROXY_BEYLA]['RPS']]
sampling_chainscope_data = [results[TestNum.SAMP_100_PROXY_EVENT]['RPS'], results[TestNum.SAMP_10_PROXY_EVENT]['RPS'], results[TestNum.SAMP_1_PROXY_EVENT]['RPS']]
...
plot_line(ax,
          ...
          legend_col=2,
          palette='tab10')
#plt.savefig(f'{plots_dir}/nginx.rps.png', dpi=300)
plt.savefig(f'{plots_dir}/nginx.rps.pdf')

...
deploy_beyla() {
  ...
  echo "wait for 20 seconds for Beyla to start..."
  sleep 20
  echo -n "${FRST}"
...
benchmark_throughput_median() {
  ...
  for i in $(seq 1 "$n"); do
    ...
    # Run the benchmark
    ab -k -r -q -d -c 32 -n 200000 "http://$ip:$port/$service_path" > "$tmp_file"
    # fortio load -c 1000 -t 60s -quiet -qps 0 'http://10.10.1.120:30688/recommendations?require=price&lat=37.883&lon=-122.252'

    # fortio load -c 1000 -t 60s -n 300000 -quiet -qps 0 'http://10.10.1.120:30688/recommendations?require=price&lat=37.883&lon=-122.252'
...
benchmark_accuracy_routine() { #parameters srv proxy file ip port
  ...
  # do not use -k option
  # fortio load -c $concurrent_users -t 60s -n 300000 -quiet -qps 0 'http://10.10.1.120:30818/recommendations?require=price&lat=37.883&lon=-122.252'
  # ab  -r -q -d -c $concurrent_users -t 60s -n 300000 http://$ip:$port/$service_path | tee -a "$output_file"
  # if concurrent_users < 10
  # do fortio load -c $concurrent_users -t 60s -quiet -qps 0 http://$ip:$port/$service_path 
  # else
  # do fortio load -c $concurrent_users -n 300000 -quiet -qps 0 http://$ip:$port/$service_path 
  # fi
  if [ $concurrent_users -lt 10 ]
  then
    fortio load -c $concurrent_users -t 60s -quiet -qps 0 http://$ip:$port/$service_path 
  else
    fortio load -c $concurrent_users -n 300000 -quiet -qps 0 http://$ip:$port/$service_path 
  fi

  # fortio load -c $concurrent_users -t 60s -n 300000 -quiet -qps 0 http://$ip:$port/$service_path 
  sleep 10
...
benchmark_bpf_cpu() {
  ...
  port=$(kubectl get svc frontend -n hotel-test -o json | jq -r '.spec.ports[] | select(.name=="5000") | .nodePort')
  local service_path="recommendations?require=price&lat=37.883&lon=-122.252"
  ab -k -r -q -d -c 32 -t $((perf_duration+10))s -n 9999999 http://$ip:$port/$service_path &>/dev/null &
  sleep 5
...
output_file="$output_dir"/loss.out
mkdir -p "$output_dir"
touch "$output_file"
...
((test_no++))
echo "[Test $test_no] beyla,sampling_rate=1, concurrency=1" | tee -a "$output_file"
if [ "${tests[test_no-1]}" == true ]; then
  # deploy_test_application false 0
  deploy_beyla 1
  # shellcheck disable=SC2046
  benchmark_accuracy_routine "$p_node_ip" 30555 1
  echo "Test complete! wait data write to collector..." | tee -a "$output_file"
  sleep 30
  echo "Test complete! Collecting results..."
  python3 scripts/extract_traces_from_jaeger.py --type beyla | tee -a "$output_file"
  # delete_test_application
  delete_beyla
else
...
((test_no++))
echo "[Test $test_no] beyla,sampling_rate=1, concurrency=10" | tee -a "$output_file"
if [ "${tests[test_no-1]}" == true ]; then
  # deploy_test_application false 0
  deploy_beyla 1
  # shellcheck disable=SC2046
  benchmark_accuracy_routine "$p_node_ip" 30555 10
  echo "Test complete! wait data write to collector..." | tee -a "$output_file"
  sleep 30
  echo "Test complete! Collecting results..."
  python3 scripts/extract_traces_from_jaeger.py --type beyla | tee -a "$output_file"
...
((test_no++))
echo "[Test $test_no] beyla,sampling_rate=1, concurrency=100" | tee -a "$output_file"
if [ "${tests[test_no-1]}" == true ]; then
  # deploy_test_application false 0
  deploy_beyla 1
  # shellcheck disable=SC2046
  benchmark_accuracy_routine "$p_node_ip" 30555 100
  echo "Test complete! wait data write to collector..." | tee -a "$output_file"
  sleep 30
  echo "Test complete! Collecting results..."
  python3 scripts/extract_traces_from_jaeger.py --type beyla | tee -a "$output_file"
...
((test_no++))
echo "[Test $test_no] beyla,sampling_rate=1, concurrency=500" | tee -a "$output_file"
if [ "${tests[test_no-1]}" == true ]; then
  # deploy_test_application false 0
  deploy_beyla 1
  # shellcheck disable=SC2046
  benchmark_accuracy_routine "$p_node_ip" 30555 500
  echo "Test complete! wait data write to collector..." | tee -a "$output_file"
  sleep 30
  echo "Test complete! Collecting results..."
  python3 scripts/extract_traces_from_jaeger.py --type beyla | tee -a "$output_file"
...
((test_no++))
echo "[Test $test_no] beyla,sampling_rate=1, concurrency=1000" | tee -a "$output_file"
if [ "${tests[test_no-1]}" == true ]; then
  # deploy_test_application false 0
  deploy_beyla 1
  # shellcheck disable=SC2046
  benchmark_accuracy_routine "$p_node_ip" 30555 1000
  echo "Test complete! wait data write to collector..." | tee -a "$output_file"
  sleep 30
  echo "Test complete! Collecting results..."
  python3 scripts/extract_traces_from_jaeger.py --type beyla | tee -a "$output_file"
...
((test_no++))
echo "[Test $test_no] beyla,sampling_rate=1, concurrency=5000" | tee -a "$output_file"
if [ "${tests[test_no-1]}" == true ]; then
  # deploy_test_application false 0
  deploy_beyla 1
  # shellcheck disable=SC2046
  benchmark_accuracy_routine "$p_node_ip" 30555 5000
  echo "Test complete! wait data write to collector..." | tee -a "$output_file"
  sleep 30
  echo "Test complete! Collecting results..."
  python3 scripts/extract_traces_from_jaeger.py --type beyla | tee -a "$output_file"
...
((test_no++))
echo "[Test $test_no] beyla,sampling_rate=1, concurrency=10000" | tee -a "$output_file"
if [ "${tests[test_no-1]}" == true ]; then
  # deploy_test_application false 0
  deploy_beyla 1
  # shellcheck disable=SC2046
  benchmark_accuracy_routine "$p_node_ip" 30555 10000
  echo "Test complete! wait data write to collector..." | tee -a "$output_file"
  sleep 30
  echo "Test complete! Collecting results..."
  python3 scripts/extract_traces_from_jaeger.py --type beyla | tee -a "$output_file"
...
((test_no++))
echo "[Test $test_no] beyla,sampling_rate=0.1, concurrency=1" | tee -a "$output_file"
if [ "${tests[test_no-1]}" == true ]; then
  # deploy_test_application false 0
  deploy_beyla 0.1
  # shellcheck disable=SC2046
...
((test_no++))
echo "[Test $test_no] beyla,sampling_rate=0.1, concurrency=10" | tee -a "$output_file"
if [ "${tests[test_no-1]}" == true ]; then
  # deploy_test_application false 0
  deploy_beyla 0.1
  # shellcheck disable=SC2046
...
((test_no++))
echo "[Test $test_no] beyla,sampling_rate=0.1, concurrency=100" | tee -a "$output_file"
if [ "${tests[test_no-1]}" == true ]; then
  # deploy_test_application false 0
  deploy_beyla 0.1
  # shellcheck disable=SC2046
...
((test_no++))
echo "[Test $test_no] beyla,sampling_rate=0.1, concurrency=500" | tee -a "$output_file"
if [ "${tests[test_no-1]}" == true ]; then
  # deploy_test_application false 0
  deploy_beyla 0.1
  # shellcheck disable=SC2046
...
((test_no++))
echo "[Test $test_no] beyla,sampling_rate=0.1, concurrency=1000" | tee -a "$output_file"
if [ "${tests[test_no-1]}" == true ]; then
  # deploy_test_application false 0
  deploy_beyla 0.1
  # shellcheck disable=SC2046
...
((test_no++))
echo "[Test $test_no] beyla,sampling_rate=0.1, concurrency=5000" | tee -a "$output_file"
if [ "${tests[test_no-1]}" == true ]; then
  # deploy_test_application false 0
  deploy_beyla 0.1
  # shellcheck disable=SC2046
...
((test_no++))
echo "[Test $test_no] beyla,sampling_rate=0.1, concurrency=10000" | tee -a "$output_file"
if [ "${tests[test_no-1]}" == true ]; then
  # deploy_test_application false 0
  deploy_beyla 0.1
  # shellcheck disable=SC2046
...
((test_no++))
echo "[Test $test_no] beyla,sampling_rate=0.01, concurrency=1" | tee -a "$output_file"
if [ "${tests[test_no-1]}" == true ]; then
  # deploy_test_application false 0
  deploy_beyla 0.01
  # shellcheck disable=SC2046
...
((test_no++))
echo "[Test $test_no] beyla,sampling_rate=0.01, concurrency=10" | tee -a "$output_file"
if [ "${tests[test_no-1]}" == true ]; then
  # deploy_test_application false 0
  deploy_beyla 0.01
  # shellcheck disable=SC2046
...
((test_no++))
echo "[Test $test_no] beyla,sampling_rate=0.01, concurrency=100" | tee -a "$output_file"
if [ "${tests[test_no-1]}" == true ]; then
  # deploy_test_application false 0
  deploy_beyla 0.01
  # shellcheck disable=SC2046
...
((test_no++))
echo "[Test $test_no] beyla,sampling_rate=0.01, concurrency=500" | tee -a "$output_file"
if [ "${tests[test_no-1]}" == true ]; then
  # deploy_test_application false 0
  deploy_beyla 0.01
  # shellcheck disable=SC2046
...
((test_no++))
echo "[Test $test_no] beyla,sampling_rate=0.01, concurrency=1000" | tee -a "$output_file"
if [ "${tests[test_no-1]}" == true ]; then
  # deploy_test_application false 0
  deploy_beyla 0.01
  # shellcheck disable=SC2046
...
((test_no++))
echo "[Test $test_no] beyla,sampling_rate=0.01, concurrency=5000" | tee -a "$output_file"
if [ "${tests[test_no-1]}" == true ]; then
  # deploy_test_application false 0
  deploy_beyla 0.01
  # shellcheck disable=SC2046
...
((test_no++))
echo "[Test $test_no] beyla,sampling_rate=0.01, concurrency=10000" | tee -a "$output_file"
if [ "${tests[test_no-1]}" == true ]; then
  # deploy_test_application false 0
  deploy_beyla 0.01
  # shellcheck disable=SC2046
...
# echo ""
# echo "--- Benchmarking with otel-auto ---"
# echo ""

# ((test_no++))
# echo "[Test $test_no] otel-auto,sampling_rate=1, concurrency=1" | tee -a "$output_file"
# if [ "${tests[test_no-1]}" == true ]; then
#   deploy_test_application false 0
#   deploy_otel_auto_go 100
#   # shellcheck disable=SC2046
#   benchmark_accuracy_routine "$p_node_ip" 30555 1
#   echo "Test complete! wait data write to collector..." | tee -a "$output_file"
#   sleep 30
#   echo "Test complete! Collecting results..."
#   python3 scripts/extract_traces_from_jaeger.py --type otel-auto | tee -a "$output_file"
#   delete_otel_auto_go
#   delete_test_application
# else
#   echo -e "${FYLW} --- Test skipped --- ${FRST}" | tee -a "$output_file"
...
# ((test_no++))
# echo "[Test $test_no] otel-auto,sampling_rate=1, concurrency=10" | tee -a "$output_file"
# if [ "${tests[test_no-1]}" == true ]; then
#   deploy_test_application false 0
#   deploy_otel_auto_go 100
#   # shellcheck disable=SC2046
...
((test_no++))
echo "[Test $test_no] deepflow,sampling_rate=1, concurrency=1" | tee -a "$output_file"
if [ "${tests[test_no-1]}" == true ]; then
  # deploy_test_application false 0
  deploy_deepflow
  # shellcheck disable=SC2046
  ...
  sleep 60
  echo "Test complete! Collecting results..."
  ./scripts/qurey_deepflow_accuracy.sh | tee -a "$output_file"
  delete_deepflow
  # delete_test_application
else
  echo -e "${FYLW} --- Test skipped --- ${FRST}" | tee -a "$output_file"
...
((test_no++))
echo "[Test $test_no] deepflow,sampling_rate=1, concurrency=10" | tee -a "$output_file"
if [ "${tests[test_no-1]}" == true ]; then
  # deploy_test_application false 0
  deploy_deepflow
  # shellcheck disable=SC2046
...
((test_no++))
echo "[Test $test_no] deepflow,sampling_rate=1, concurrency=100" | tee -a "$output_file"
if [ "${tests[test_no-1]}" == true ]; then
  # deploy_test_application false 0
  deploy_deepflow
  # shellcheck disable=SC2046
...
((test_no++))
echo "[Test $test_no] deepflow,sampling_rate=1, concurrency=500" | tee -a "$output_file"
if [ "${tests[test_no-1]}" == true ]; then
  # deploy_test_application false 0
  deploy_deepflow
  # shellcheck disable=SC2046
...
((test_no++))
echo "[Test $test_no] deepflow,sampling_rate=1, concurrency=1000" | tee -a "$output_file"
if [ "${tests[test_no-1]}" == true ]; then
  # deploy_test_application false 0
  deploy_deepflow
  # shellcheck disable=SC2046
...
((test_no++))
echo "[Test $test_no] deepflow,sampling_rate=1, concurrency=5000" | tee -a "$output_file"
if [ "${tests[test_no-1]}" == true ]; then
  # deploy_test_application false 0
  deploy_deepflow
  # shellcheck disable=SC2046
...
((test_no++))
echo "[Test $test_no] deepflow,sampling_rate=1, concurrency=10000" | tee -a "$output_file"
if [ "${tests[test_no-1]}" == true ]; then
  # deploy_test_application false 0
  deploy_deepflow
  # shellcheck disable=SC2046
...
# # ((test_no++))
# echo "[Test $test_no] application injection,sampling_rate=1, concurrency=1" | tee -a "$output_file"
# if [ "${tests[test_no-1]}" == true ]; then
#   delete_chain_scope
...
# ((test_no++))
# echo "[Test $test_no] application injection,sampling_rate=0.1, concurrency=1" | tee -a "$output_file"
# if [ "${tests[test_no-1]}" == true ]; then
#   delete_chain_scope
...
# ((test_no++))
# echo "[Test $test_no] application injection,sampling_rate=0.01, concurrency=1" | tee -a "$output_file"
# if [ "${tests[test_no-1]}" == true ]; then
#   delete_chain_scope
...
((test_no++))
echo "[Test $test_no] ip tagging,sampling_rate=1, concurrency=1" | tee -a "$output_file"
if [ "${tests[test_no-1]}" == true ]; then
  delete_chain_scope
...
((test_no++))
echo "[Test $test_no] ip tagging,sampling_rate=1, concurrency=5000" | tee -a "$output_file"
if [ "${tests[test_no-1]}" == true ]; then
  delete_chain_scope
  deploy_chain_scope false true 0 "" "$(get_entrypoint_ips)" 1 false true 2
  ./scripts/utils/set_sampling_rate.sh 1
...
((test_no++))
echo "[Test $test_no] ip tagging,sampling_rate=1, concurrency=10000" | tee -a "$output_file"
if [ "${tests[test_no-1]}" == true ]; then
  delete_chain_scope
  deploy_chain_scope false true 0 "" "$(get_entrypoint_ips)" 1 false true 2
  ./scripts/utils/set_sampling_rate.sh 1
...
((test_no++))
echo "[Test $test_no] ip tagging,sampling_rate=0.1, concurrency=1" | tee -a "$output_file"
if [ "${tests[test_no-1]}" == true ]; then
  delete_chain_scope
...
((test_no++))
echo "[Test $test_no] ip tagging,sampling_rate=0.1, concurrency=5000" | tee -a "$output_file"
if [ "${tests[test_no-1]}" == true ]; then
  delete_chain_scope
  deploy_chain_scope false true 0 "" "$(get_entrypoint_ips)" 10 false true 2
  ./scripts/utils/set_sampling_rate.sh 10
...
((test_no++))
echo "[Test $test_no] ip tagging,sampling_rate=0.1, concurrency=10000" | tee -a "$output_file"
if [ "${tests[test_no-1]}" == true ]; then
  delete_chain_scope
  deploy_chain_scope false true 0 "" "$(get_entrypoint_ips)" 10 false true 2
  ./scripts/utils/set_sampling_rate.sh 10
...
((test_no++))
echo "[Test $test_no] ip tagging,sampling_rate=0.01, concurrency=1" | tee -a "$output_file"
if [ "${tests[test_no-1]}" == true ]; then
  delete_chain_scope
...
((test_no++))
echo "[Test $test_no] ip tagging,sampling_rate=0.01, concurrency=5000" | tee -a "$output_file"
if [ "${tests[test_no-1]}" == true ]; then
  delete_chain_scope
  # deploy_test_application false 0
  deploy_chain_scope false true 0 "" "$(get_entrypoint_ips)" 100 false true 2
...
((test_no++))
echo "[Test $test_no] ip tagging,sampling_rate=0.01, concurrency=10000" | tee -a "$output_file"
if [ "${tests[test_no-1]}" == true ]; then
  delete_chain_scope
  # deploy_test_application false 0
  deploy_chain_scope false true 0 "" "$(get_entrypoint_ips)" 100 false true 2

...
delete_beyla() {
  ...
  kubectl -n beyla wait pod all --for=delete &>/dev/null
}
...
delete_deepflow() {
  local clickhouse_node_ip=$(get_deepflow_clickhouse_node_ip)
  local mysql_node_ip=$(get_deepflow_mysql_node_ip)
...
benchmark_accuracy_routine() { #parameters srv proxy file ip port
  ...
  # do not use -k option
  ab  -r -q -d -c $concurrent_users -t 60 -n 9999999 http://$ip:$port/$service_path | tee -a "$output_file"
  sleep 10

...
class Trace:
    ...
    def _try_build_span_tree(self):
        ...
        with self.lock:
            ...
            while len(queue) > 0:
                spanid = queue.pop(0)
                if spanid in self.processed_span_index:
...
class SimpleLogHandler:
            ...
            for resource_logs in log_request.resource_logs:
                ...
                for scope_logs in resource_logs.scope_logs:
                    ...
                    for log_record in scope_logs.log_records:
                        self._process_log_record(log_record.body.string_value, resource_info, scope_info)
    ...
    def process_export_json(self, request_data):
        """Handle exported log data"""
        # Process received log data
        for resource_logs in request_data.get("resourceLogs", []):
            # Process log records
            for scope_logs in resource_logs.get("scopeLogs", []):
                if scope_logs["scope"]["name"] == "tcp_event":
                    for log_record in scope_logs.get("logRecords", []):
                        self._process_log_record(log_record["body"].get("stringValue"), None, None)
    ...
    def _process_log_record(self, log_record_string_value, resource_info, scope_info):
        """Process a single log record by adding it to the event queue"""
        event_data = json.loads(log_record_string_value)
        # Build event object
        event = {
...
def handle_logs():
    ...
    try:
        ...
        log_handler.process_export_json(request_data)
